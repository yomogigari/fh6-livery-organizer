...
EXPECTED_VERSION = "0.4.60-r09"
HISTORICAL_FEATURE_TESTS = (
    ROOT / "tests" / "test_i18n.py",
    ROOT / "tests" / "test_vehicle_metadata_update_dialog_r04.py",
    ROOT / "tests" / "test_fh6_move_target_visibility_r05.py",
    ROOT / "tests" / "test_creator_colors_r06.py",
    ROOT / "tests" / "test_css_theme_tokens_r07.py",
    ROOT / "tests" / "test_dark_creator_color_visibility_r08.py",
)
...
class CurrentVersionTests(unittest.TestCase):
    ...
    def test_previous_revision_is_not_still_current(self) -> None:
        self.assertNotIn('VERSION = "0.4.60-r08"', self.text[:10000])
        self.assertNotIn("Livery Organizer for FH6 v0.4.60-r08", self.text[:400])
