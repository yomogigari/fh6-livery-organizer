...
**v0.4.62 Preview**（2026-09-18公開）では、比較画面と再ダウンロード重複整理から「FH6で削除済み（仮）」へ直接反映できるようにし、仮削除済み一覧の検索・復元を強化しました。さらに、保存済み `C_livery` の構造から5項目の統計的特徴を確認する**作成方法監査スコア**を追加し、カード・比較・CSV / Excelで `0/5`～`5/5` の数値として確認できます。監査スコアは作成方法の判定結果、自動生成の確率や信頼度、ペイント品質、作者を評価する値ではありません。
