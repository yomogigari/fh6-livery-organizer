...
# -*- coding: utf-8 -*-
"""
Livery Organizer for FH6 v0.4.61-r08
================================
...
<main>
  ...
  <section id="fh6MyDesignSection" class="fh6-my-design-section hidden" aria-label="FH6マイデザイン順">
    ...
    <div class="fh6-my-design-foot small">
      <p><b class="fh6-foot-label">表示と基本移動</b>1 列に上段と下段の 2 件を表示します。横スクロール、通常のマウスホイール（縦回転）、← / → キー、「前へ / 次へ」のいずれでも 1 列ずつ移動できます。端では反対側へ循環します。各カードの下部には、FH6 の「マイデザイン」に表示される日付を DD/MM/YYYY 形式で表示します。</p>
      <p><b class="fh6-foot-label">位置・車種ジャンプ</b><b>537 / #537</b> の実スロット通し番号、<b>#269U / #269D</b> の列＋U/D 位置（U = 上段、D = 下段）、または <b>RX-7</b> のような車種名を指定できます。車種名は年式・メーカー・車名を対象に部分一致し、↑ / ↓ で候補を選択できます。候補を選ばずに Enter または「移動」を押すと、一致した各車種を1車種1位置ずつ巡回します。候補を選んで Enter すると、その1車種に属する実スロットを巡回します。検索を確定した後は <b>Shift+→</b> で次の一致、<b>Shift+←</b> で前の一致へ移動し、末尾では先頭へ循環します。<b>J</b> でジャンプ入力欄へ戻れます。</p>
  ...
  <div class="modal-panel help-panel" role="dialog" aria-modal="true" aria-labelledby="helpTitle">
    <div class="modal-head">
      <div>
        ...
        <div class="small">v{VERSION} 現在仕様 / 操作の流れに沿ったガイド</div>
      </div>
      <button data-close-modal="helpModal">閉じる</button>
    </div>

    <div class="help-intro">
      <b>FH6 で取得したペイントを、検索・比較・整理・バックアップするためのレポートです。</b>
      判定、タグ、メモ、お気に入り、後で確認、作成者カラーなどはブラウザ側に保存します。
      HTML 上の操作は、FH6 の GameSave を削除・移動・リネーム・上書きしません。
    </div>

    <section class="help-fh6-move-workflow" aria-labelledby="helpFh6MoveWorkflowTitle">
      <h4 id="helpFh6MoveWorkflowTitle">Organizer で目的のデザインを探し、FH6 上の位置へ移動する</h4>
      <ol>
        <li>車種・メーカー・年式・作成者・タイトルなどから目的のペイントを探します。カードまたは比較画面の <b>#603 / #302U</b> などをクリックすると、FH6 移動対象として設定できます。</li>
        <li><b>F</b> キー、または <b>FH6で選択デザインへ移動</b> を実行します。</li>
        <li>Navigator Bridge は FH6 を前面に切り替え、設定した間隔でキー入力を送り、選択した位置までカーソルを移動します。到着後の読み込み・確認・削除・確定は、利用者が FH6 で行います。</li>
      </ol>
    </section>
    ...
    <div class="help-quick-start" aria-label="基本操作">
      <div class="help-step">
        ...
        <p>検索、並び替え、上部のクイック絞り込みから対象を探します。</p>
      </div>
      <div class="help-step">
        <span class="help-step-number">2</span><b>絞る</b>
        <p>整理状態、車種進捗、件数、バイナル数、タグ、新規などの条件を組み合わせます。</p>
      </div>
      <div class="help-step">
        <span class="help-step-number">3</span><b>整理する</b>
        <p>残す / 削除候補 / 未決定を設定し、必要に応じてタグ・メモ・各フラグを付けます。</p>
      </div>
      <div class="help-step">
        <span class="help-step-number">4</span><b>保存・出力</b>
        <p>作業の区切りでユーザーデータを保存し、必要に応じて CSV / Excel を出力します。</p>
      </div>
    </div>

    <div class="help-current-note">
      確認したい内容に合わせて、下の 4 カテゴリを切り替えてください。
    </div>
    ...
    <div id="helpBasicsPane" class="help-pane" role="tabpanel" aria-labelledby="helpBasicsTab">
      <section class="help-section">
        <h4>画面上部とクイック絞り込み</h4>
        <p>
          画面上部には、総ペイント数、車種数、メーカー数、作成者数、年式範囲を表示します。
          未決定・お気に入り・後で確認・新規・類似候補の現在件数も確認できます。
        </p>
        <p>
          <b>車種数 / メーカー / 作成者数 / 年式</b> は、候補から値を選ぶクイック絞り込みです。
          候補検索、複数選択、「選択中のみ」「表示候補をすべて選択」「この項目を解除」を利用できます。
          <b>未決定 / お気に入り / 後で確認 / 新規 / 類似候補</b> をクリックすると、対応する絞り込み条件を直接 ON / OFF できます。
        </p>
        <div class="help-tip">
          同じ項目で複数候補を選ぶと <b>OR</b>、異なる項目を組み合わせると <b>AND</b> で絞り込みます。
        </div>
      </section>
      ...
      <section class="help-section">
        ...
        <p>
          通常検索は部分一致です。
          車両情報、タイトル、説明、作成者、Car ID、取得情報に加え、ブラウザで入力したタグとメモも検索します。
        </p>
        <table class="help-mini-table">
          <tbody>
            <tr><th><code>v:</code></th><td>Vehicle。数字は Car ID と完全一致し、文字列は車名・メーカー・モデル・年式を検索します。</td></tr>
            <tr><th><code>c:</code></th><td>Creator。作成者を検索します。</td></tr>
            <tr><th><code>n:</code></th><td>Name。タイトルを検索します。</td></tr>
            <tr><th><code>d:</code></th><td>Description。説明を検索します。</td></tr>
            <tr><th><code>t:</code></th><td>Tag。ブラウザで設定したタグを検索します。</td></tr>
          </tbody>
        </table>
        <p>
          例: <code>v:0363</code>、<code>v:Porsche</code>、
          <code>c:DemoWorks n:&quot;デモペイント 001&quot;</code>、<code>t:デモ c:DemoWorks</code>。
          空白を含む値はダブルクォートで囲みます。複数の条件を指定した場合は AND 検索です。
        </p>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <table class="help-mini-table">
          ...
          <tbody>
            <tr><td>メーカー順 / 車名順 / 年式順</td><td>車種グループを保ったまま、それぞれの基準で並び替えます。年式を取得できなかった車種は、年式順では最後に表示します。</td></tr>
            <tr><td>マイデザイン順</td><td>FH6 の「マイデザイン」と同じ基準で、Car ID の小さい車種から並べます。同じ車種では取得日時の古い順です。カード左上には <b>#001</b> からの通し番号を表示し、絞り込み中も番号を詰め直しません。FH6 側に昇順 / 降順の切り替えがないため、この並び順も固定です。</td></tr>
            <tr><td>FH6マイデザイン順</td><td>
              <div class="help-sort-fh6-detail">
                <p><b>表示</b><br>FH6 に近い <b>2 段の横スクロール</b>で表示します。1 列目は <b>#001U / #001D</b>、2 列目は <b>#002U / #002D</b> のように位置を示します。カード内は <b>タイトル → 作成者 → 日付</b> の順です。完全一致の再ダウンロードも別ペイントとして扱い、FH6 本体と同じ実スロット位置を保ちます。</p>
                <p><b>横移動</b><br>横スクロール、通常のマウスホイール（縦回転）、← / → キー、「前へ / 次へ」のいずれでも 1 列ずつ移動できます。先頭から左へ移動すると最後へ、最後から右へ移動すると先頭へ循環します。</p>
                <p><b>位置・車種ジャンプ</b><br><b>537 / #537</b> の実スロット番号、<b>#269U / #269D</b> の列＋U/D 位置、または車種名の一部を指定できます。車種候補は年式・メーカー・車名を対象に部分一致し、↑ / ↓ で選択できます。候補を選ばずに Enter または「移動」を押すと、一致した各車種を1車種1位置ずつ巡回します。候補を選んで Enter すると、その1車種に属する実スロットを巡回します。検索を確定した後は <b>Shift+← / Shift+→</b> で前後の一致へ移動し、末尾では先頭へ循環します。<b>J</b> で入力欄へ戻り、Esc で候補を閉じます。</p>
                <p><b>FH6で選択デザインへ移動</b><br>Organizer で車種・メーカー・年式・作成者・タイトルなどから目的のペイントを探し、カードの <b>#603</b> や <b>#302U</b> をクリックして FH6 移動対象に設定します。他の並び順、類似ペイント比較、再 DL 重複整理画面でも同じ操作ができます。選択後は「FH6で選択デザインへ移動」または <b>F</b> キーを実行します。Bridge は対象位置までのカーソル移動だけを補助し、読み込み・選択・削除・確定は行いません。入力の取りこぼしで位置がずれる場合は、FH6 移動設定の待ち時間を長くしてください。</p>
                <p><b>FH6で削除済み（仮）</b><br>FH6 で利用者が不要なデザインを削除した後、同じデザインを Organizer で一時的に非表示にすると、残りの実スロット番号と列＋U/D 位置を詰め直します。再計算した位置は次の Bridge 移動にも使います。上部の <b>FH6削除済み（仮）</b> から個別復元または全件復元ができます。復元画面では、車種・タイトル・作成者・元の実スロット番号 / FH6 位置で検索でき、FH6 表示日付と取得日時も確認できます。仮削除は現在の生成 HTML 専用で、新しい HTML には引き継ぎません。</p>
                <p><b>絞り込み中</b><br>仮削除を反映した現在位置を維持し、表示対象がない列だけを省略します。ジャンプ先が絞り込みで非表示の場合は、条件を変更せずに案内を表示します。移動先は一時的に強調します。</p>
              </div>
            </td></tr>
            <tr><td>作成者順</td><td>作成者ごとにグループ化して表示します。</td></tr>
            <tr><td>タイトル順 / 整理状態順</td><td>車種グループをまたいでカード単位で並び替えます。整理状態順は、未決定 → 後で確認 → 残す → 削除候補の順です。</td></tr>
            <tr><td>整理進捗順</td><td>車種単位で、未着手（0%）/ 整理中（1～99%）/ 完了（100%）の順または逆順に並べます。</td></tr>
            <tr><td>ペイント件数順</td><td>車種ごとのペイント件数で車種グループを並び替えます。</td></tr>
            <tr><td>監査スコア順</td><td>作成方法監査のスコアでカードを並び替えます。スコアを算出できないカードは、高い順と低い順のどちらでも最後に表示します。</td></tr>
            <tr><td>アップロード日順</td><td>作成者が FH6 へアップロードした日付でカード単位に並び替えます。日付を取得できなかったカードは、昇順 / 降順のどちらでも最後に表示します。</td></tr>
            <tr><td>バイナル数順 / 取得日時順</td><td>カード単位で昇順 / 降順に並び替えます。取得日時は、フォルダ名の UTC 時刻を JST（UTC+9）へ変換して表示します。</td></tr>
          </tbody>
        </table>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <p>
          ...
          「条件」では、作成者、整理状態、車種の整理進捗、ペイント件数、バイナル数、タグ、新規、類似候補、お気に入り、後で確認、プリセットを設定します。
        </p>
        <ul>
          <li><b>整理状態</b>：未決定 / 残す / 削除候補を複数選択できます。</li>
          <li><b>車種の整理進捗</b>：未着手 / 整理中 / 完了を複数選択できます。</li>
          <li><b>ペイント件数</b>：車種に含まれるペイント件数を複数選択できます。</li>
          <li><b>バイナル数</b>：最小 / 最大を範囲指定できます。片側だけの指定も可能です。</li>
          <li><b>タグ</b>：複数選択し、「いずれか（OR）」/「すべて（AND）」を切り替えます。</li>
          <li><b>新規のみ</b>：手動保存した新規判定基準より後に現れたカードだけを表示します。HTML を開くだけでは基準を更新しません。</li>
        </ul>
        <p>
          現在の条件は画面上部の条件チップに表示し、× で個別に解除できます。
          「絞り込み解除」は、検索を含む主要条件をまとめて解除します。
          よく使う条件は、名前付きプリセットとして保存・適用・削除できます。
        </p>
      </section>
    </div>
    ...
    <div id="helpOrganizePane" class="help-pane hidden" role="tabpanel" aria-labelledby="helpOrganizeTab">
      <section class="help-section">
        <h4>カードの見方と整理</h4>
        <p>
          各カードには、サムネイル、車両情報、タイトル、説明、作成者、アップロード日、取得日時、バイナル数などを表示します。
          サムネイルをクリックすると拡大できます。
        </p>
        <ul>
          <li><b>残す / 削除候補 / 未決定</b>：整理状態を設定します。</li>
          <li><b>お気に入り</b>：残したい、またはよく使う候補の目印です。</li>
          <li><b>後で確認</b>：判断を保留したいカードの目印です。</li>
          <li><b>タグ・メモ</b>：任意の分類と自由記述です。どちらも検索対象になります。</li>
          ...
          <li><b>作成者カラー</b>：作成者名の横にある色ボタンから設定します。同じ作成者名の全カードへ反映し、現在のペイントが 0 件になっても設定を保持します。</li>
        </ul>
        <div class="help-tip">
          <b>作成者カラー管理</b>：
          <span class="help-path"><span>その他の操作</span> → <span>作成者カラー管理</span></span>
          では、作成者カラーを一覧から検索・変更・解除できます。「設定済みのみ」を選ぶと、色を設定した作成者だけに絞り込めます。現在のペイントが 0 件でも保存済みの色設定があれば、「現在0件で保持」として確認できます。
        </div>
        <div class="help-tip help-warning">
          <b>監査スコアは 5 項目のうち条件に一致した項目数です。</b>
          数値だけでは作成方法を判定できません。
          解析に必要な情報が不足する場合は <b>—</b> を表示します。
          カードの監査欄を開くと、一致した条件と外れた条件を確認できます。
        </div>
        <div class="help-tip help-warning">
          <b>「削除候補」は Organizer 内のラベルです。</b>
          FH6 のファイルを削除しません。
        </div>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <div class="help-tip help-warning">
          各項目は作成方法を直接判定しません。
          手作業で作成したペイントが条件に一致する場合も、自動生成を含むペイントが条件に一致しない場合もあります。
          監査スコアは、ペイントデータに見られる統計的特徴を比較するための参考値として使用してください。
        </div>
      </section>
      ...
      <section class="help-section">
        ...
        <p>
          カード左上のチェックは、一括操作の対象を選ぶために使います。
          <span class="help-path">絞り込み → 選択・一括</span> では、表示中を選択、選択反転、選択解除、<b>選択中のペイント比較</b>、一括判定、一括お気に入り、一括「後で確認」、タグ管理を利用できます。
        </p>
        <p>
          タグ管理は <b>追加 / 削除 / 置換</b> に対応します。
          追加では、カンマ区切りで複数のタグを指定できます。
          編集操作は「元に戻す / やり直す」の対象です。
          <span class="help-path">その他の操作 → メンテナンス → 操作履歴</span> で履歴を確認できます。
        </p>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <p>
          ...
          「残す」または「削除候補」を整理済み、「未決定」を未整理として扱い、未着手（0%）/ 整理中（1～99%）/ 完了（100%）に分類します。
        </p>
        <p>
          画面上部の「車種整理」バーには、未着手 / 整理中 / 完了の車種数を表示します。
          各ボタンを押すと、その状態だけに絞り込めます。
          <b>未完了車種のみ</b> は、未着手と整理中をまとめて表示します。
          <b>前の未完了車種へ / 次の未完了車種へ</b> は、現在の検索・絞り込み・並び順を基準に未完了車種を前後へ巡回し、その車種の未決定カードを優先してフォーカスします。
          <span class="help-path">その他の操作 → 連続整理</span> の <b>最初の未完了車種へ</b> では、現在の表示順の先頭にある未完了車種へ戻れます。
        </p>
        <p>
          車種の進捗バッジをクリックすると、他の条件を解除してその車種だけを表示します。
          移動中の車種は見出しを強調し、<b>「未完了 3 / 18車種 · 全 76車種」</b> のように現在位置を表示します。
          現在車種を完了すると <b>「現在 完了」</b> へ変わります。
          検索・絞り込み・並び替えを変えた場合は、現在の表示順で位置を再計算します。
        </p>
        <div class="help-tip">
          全ペイントの未決定が 0 件になると、上部に <b>整理完了</b> を表示します。
          同じ場所で、残す件数 / 削除候補件数とユーザーデータのバックアップ状態も確認できます。
        </div>
      </section>

      <section class="help-section">
        <h4>類似候補と再ダウンロード重複</h4>
        <p>
          類似候補があるカードには、判定理由に応じて <b>「画像一致 ○件」</b> または <b>「同一作者・同名 ○件」</b> と表示します。
          同じ車種内の <b>サムネイル完全一致</b> または <b>作成者＋タイトル一致</b> を手がかりに候補化します。
          比較画面では、一致理由、取得日時、説明、整理状態、フィンガープリントを確認し、各ペイントの <b>残す / 削除候補 / 未決定</b> をその場で変更できます。
          比較画面の変更は一覧カードと同じ保存・Undo 経路を使うため、閉じた後の一覧にも反映します。
        </p>
        <p>
          <span class="help-path">絞り込み → 条件</span> では、<b>類似候補のみ</b>、<b>画像一致のみ</b>、<b>同一作者・同名のみ</b> を使って判定理由ごとに絞り込めます。
          同じペイントを再ダウンロードした完全一致スロットが現在のスナップショット内に複数ある場合は、各スロットを別カードとして表示し、<b>再DL重複</b> バッジを付けます。
          <b>再DL重複のみ</b> を使うと、この完全一致再ダウンロードだけを表示できます。
        </p>
        <p>
          <b>再DL重複 ○件</b> バッジをクリックすると専用整理画面を開きます。
          FH6 位置、FH6 表示日付、取得日時、Livery ID を見比べて <b>「これを残す」</b> を選ぶと、その 1 件を「残す」、同じ完全一致グループの残りを「削除候補」にまとめて設定します。
          FH6 本体で不要な重複を削除した後は、同じ画面の <b>FH6で削除済み</b> を押すと仮削除へ反映し、残りの実スロット位置を再計算します。
          1 件だけ残ったグループは整理済みとして専用画面から外れます。
        </p>
        <p>
          カードを 2 件以上チェックして <span class="help-path">絞り込み → 選択・一括 → 選択中を比較</span> を使うと、自動類似判定に入らない任意のペイントも同じ比較画面で確認できます。
          「古い候補を選択（最新以外）」は類似候補比較のときだけ表示し、Organizer 上の一括操作対象を選ぶだけです。GameSave は変更しません。
          車種・作成者リンクをダブルクリックすると、件数や平均バイナル数などの詳細を確認できます。
        </p>
      </section>
      ...
      <section class="help-section">
        ...
        <p>
          「次の未決定へ」と、未完了車種の「最初 / 前 / 次」を利用できます。
          <b>連続整理: ON</b> では、<kbd>K</kbd> / <kbd>D</kbd> で判定した直後に、現在の表示順で次の未決定カードへ自動移動します。
          <kbd>U</kbd> では自動移動しません。
        </p>
      </section>
    </div>
    ...
    <div id="helpDataPane" class="help-pane hidden" role="tabpanel" aria-labelledby="helpDataTab">
      <section class="help-section help-section-wide">
        ...
        <p>
          <b>「概要・進捗」</b>では、保存領域、件数、重複、車両 DB、最終スキャン、サムネイル方式、ビルド、UI 状態、保存方式、整理件数・整理進捗・選択数・新規判定基準を確認できます。
          下部の <b>「利用準備」</b>では、ブラウザ保存、レポートデータ整合性、サムネイル方式を確認し、そのまま動作診断へ進めます。
          「表示 / 残す / 削除候補 / 未決定」の件数ボタンは絞り込みにも使えます。
        </p>
        <p>
          <b>「前回との差分」</b>では、保存済みの新規判定基準と今回の HTML を比較し、<b>新規</b>と<b>消滅</b>を同じ画面に表示します。
          「新規」は現在のカード、「消滅」は基準には存在したものの今回の HTML には存在しないペイントです。
          基準を更新するときだけ「今回を新規判定の基準にする」を押します。
        </p>
        <p>
          <b>「バックアップ状態」</b>では、ユーザーデータ保存 / 判定バックアップの前回保存時刻と、現在との差を確認します。
      ...
      <section class="help-section">
        <h4>ユーザーデータ保存・復元</h4>
        <p>
          判定、タグ、メモ、お気に入り、後で確認、作成者カラー、UI 状態、絞り込みプリセットなどは、通常ブラウザの <code>localStorage</code> に保存します。
          <code>localStorage</code> を利用できない環境では、一時メモリへ切り替わります。
        </p>
        <p>
          <b>ユーザーデータ保存</b>は、判定・メタデータ・作成者カラーに加え、UI 状態とスキャン状態を JSON へ保存します。
          <b>ユーザーデータ復元</b>では、プレビュー後に「現在データと統合」または「完全に置換」を選べます。
          <b>判定バックアップ</b>は、主に残す / 削除候補の判定だけを保存する軽量版です。
        </p>
        <p>
          「統合」は現在データを残し、ファイル内の値だけを上書きします。
          現在のレポートに表示されていないペイントの整理データも保存領域へ復元します。
          「完全に置換」は、現在表示されていないペイントを含む保存済みの整理状態・タグ・メモ・作成者カラーなどを消去してから復元します。
        </p>
      </section>
      ...
      <section class="help-section">
        <h4>バックアップ状態の読み方</h4>
        <p>
          状態は <b>未バックアップ / 最新 / 変更あり</b> の 3 種類です。
          ユーザーデータの比較対象は、判定・タグ・メモ・お気に入り・後で確認・作成者カラー・スキャン状態です。
          検索条件、並び順、テーマなどの日常的な UI 変更だけでは「変更あり」になりません。
        </p>
        <p>
          「前回のユーザーデータ保存からの変更」では、同じペイントのタグとメモを両方変更しても 1 件として数えます。
        </p>
        <div class="help-tip">
          表示する保存時刻は、ブラウザが JSON のダウンロードを開始した時刻です。
          HTML から、実ファイルが保存先に残っているかどうかは確認できません。
        </div>
      </section>

      <section class="help-section help-section-wide">
        <h4>出力ファイルと HTML の持ち運び</h4>
        <table class="help-mini-table">
          <thead><tr><th>出力</th><th>内容・注意点</th></tr></thead>
          <tbody>
            <tr><td>全件 CSV</td><td>全ペイントを出力します。現在の判定・タグ・メモ・お気に入り・後で確認を反映します。</td></tr>
            <tr><td>表示中 CSV</td><td>現在表示しているカードだけを、画面上の並び順と同じ行順で出力します。</td></tr>
            <tr><td>Excel</td><td>レポート生成時の <code>livery-organizer-for-fh6.xlsx</code> です。サムネイルを含みますが、HTML を開いた後の判定変更はリアルタイムには反映しません。</td></tr>
            <tr><td>ユーザーデータ JSON</td><td>判定、タグ、メモ、お気に入り、後で確認、作成者カラー、UI 状態、スキャン状態をブラウザから保存します。別 PC への整理内容の移行にも使えます。</td></tr>
            <tr><td>判定バックアップ JSON</td><td>残す / 削除候補を中心に保存する軽量バックアップです。</td></tr>
          </tbody>
        </table>
        <p>
          サムネイルを HTML に埋め込む設定なら、HTML 単体で移動できます。
          <code>thumbnails</code> 参照方式では、HTML とフォルダを一緒にコピーしてください。
          HTML だけを別 PC へ移しても元ブラウザの <code>localStorage</code> は移らないため、整理内容はユーザーデータ保存 / 復元で移行します。
        </p>
        <p>
          通常出力は HTML と Excel です。
          「解析用データを data フォルダへ出力」を有効にした場合だけ、JSON / CSV の解析用データを追加生成します。
          解析用データは HTML 表示には不要です。
        </p>
      </section>
    </div>
    ...
    <div id="helpEnvironmentPane" class="help-pane hidden" role="tabpanel" aria-labelledby="helpEnvironmentTab">
      <section class="help-section help-section-wide">
        ...
        <p>
          <b>ブラウザ保存</b>、<b>レポートデータ</b>、<b>サムネイル方式</b>を確認できます。
          「準備OK」なら通常利用できます。
          初めて開いた HTML、別フォルダへ移動した HTML、<code>thumbnails</code> 外部参照版では、続けて「動作診断」を実行すると画像の実読込まで確認できます。
        </p>
        <p>
          JavaScript エラーが発生した場合は、「レポート情報」にエラー内容と <b>動作診断</b> ボタンを表示します。
          保存方式が「一時メモリ」の場合は、タブを閉じる前にユーザーデータを保存してください。
        </p>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <p>
          デスクトップ画面の <b>実行前チェック</b>では、保存領域、FH6 本体、出力先、出力構成を確認します。
          保存領域または出力先に問題がある場合は実行を止めます。
          FH6 本体だけを検出できない場合は「要確認」とし、Car ID 表示で続行できます。
          初回起動時は <b>初回利用ガイド</b> も自動表示し、2 回目以降は画面下部の同名ボタンから開けます。
        </p>
        <p>
          出力先の保護対象は、選択した <code>pgs</code> だけではなく、その親の <b>GameSave ディレクトリ全体</b>です。
          GUI、CLI、レポート書込み処理の各段階で、GameSave 配下への出力を拒否します。
        </p>
        <p>
          Python 環境で Tkinter を読み込めず GUI を開始できない場合は、自動走査へ移行せず停止します。
          コマンドプロンプトから <code>--environment-check</code> を実行すると、走査せずに環境と設定だけを確認できます。
          車両アセット探索では、FH6 側の 0 バイトの <code>.zip</code> を空プレースホルダーとしてスキップします。
          0 バイト以外で「読み取り不可ZIP」が表示された場合は、<code>--inspect-vehicle-assets</code> で FH6 本体を変更せず、空 ZIP と該当 ZIP の相対パス・例外種別・先頭シグネチャを確認できます。
        </p>
        <div class="help-tip">
          出力構成には HTML と Excel を常に含みます。
          サムネイル埋め込みが ON の場合は HTML に画像を含め、OFF の場合は <code>thumbnails</code> フォルダも生成します。
          解析データが ON の場合だけ <code>data</code> を追加します。
        </div>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <ol>
          <li>FH6 を完全終了してから Organizer を起動します。</li>
          <li>保存領域 / FH6 本体 / 出力先を確認し、「実行前チェック」が OK または要確認になっていることを確認します。</li>
          <li>サムネイル方式と解析データ出力を選び、「ペイントデータチェック」を実行します。</li>
          <li>生成後は HTML が自動で開きます。「レポート情報 → 利用準備」を確認し、初回利用時やファイル移動後は「動作診断」を実行します。</li>
        </ol>
        <p>
          生成完了ダイアログには、その実行で作成した HTML / Excel / <code>thumbnails</code> / <code>data</code> のうち、該当する出力を表示します。
          配布・移動するときは、表示された出力単位を保ってください。
        </p>
      </section>
      ...
      <section class="help-section help-section-wide">
        ...
        <p>
          Organizer の <b>環境・サポート情報</b>では、Organizer のバージョン、Windows / Python 環境、Tkinter、設定ファイル、3 つのパス、出力構成、実行前チェック結果を 1 つのテキストにまとめて確認・コピーできます。
          不具合報告にこの情報を添えると、環境差や設定差を確認しやすくなります。
        </p>
        <div class="help-tip help-warning">
          コピー内容には Windows ユーザー名やローカルパスが含まれる場合があります。
          第三者へ送る前に内容を確認してください。
          CLI では <code>--environment-check</code> でも同じ種類の情報を表示できます。
        </div>
      </section>
      ...
      <section class="help-section">
        ...
        <table class="help-mini-table">
          <tbody>
            <tr><th>表示</th><td>ダーク / ライトテーマと、約 150 px 幅のコンパクト表示を切り替えます。コンパクト表示では、追加表示する項目を選択できます。</td></tr>
            <tr><th>連続整理</th><td>次の未決定、未完了車種の最初 / 前 / 次、K / D 後の自動移動を利用できます。</td></tr>
            <tr><th>エクスポート</th><td>全件 CSV、表示中 CSV、Excel を出力します。</td></tr>
            <tr><th>バックアップ・復元</th><td>ユーザーデータ保存 / 復元、判定バックアップ / 復元を利用できます。</td></tr>
            <tr><th>メンテナンス</th><td>操作履歴、動作診断、判定状態の全消去を利用できます。動作診断では、カード / CSV の対応、重複キー、車種件数、進捗メタデータ、サムネイル参照、マイデザイン番号、FH6 実スロット、FH6 表示日付、再 DL 重複グループを確認します。</td></tr>
          </tbody>
        </table>
        <p>
          「判定状態を全消去」は、残す / 削除候補を未決定へ戻します。
      ...
      <section class="help-section">
        <h4>スマートフォン表示</h4>
        <p>
          狭い画面では上部 UI を簡略化し、車種 / メーカー / 作成者 / 年式のクイック絞り込みと「絞り込み・並び替え」ボタンを表示します。
          車種整理バーの未完了絞り込みと、前 / 次の未完了車種移動も利用できます。
        </p>
      ...
      <section class="help-section help-section-wide">
        ...
        <div class="help-tip">
          ダイアログを開くと、フォーカスはダイアログ内へ移動します。
          <kbd>Tab</kbd> / <kbd>Shift</kbd> + <kbd>Tab</kbd> はダイアログ内を循環し、閉じると原則として開く前に操作していたボタンへ戻ります。
          <kbd>F</kbd> は、類似ペイント比較・選択比較・再 DL 重複整理を開いている間も、選択済みの FH6 移動対象へ移動できます。
          <kbd>J</kbd> と <kbd>Shift</kbd> + <kbd>← / →</kbd> は FH6 マイデザイン順でのみ利用できます。
          入力欄・テキストエリア・選択欄へフォーカスしている間は、文字キーのショートカットを無効にします。
        </div>
      </section>
      ...
      <section class="help-section help-section-wide help-safety">
        <h4>GameSave と安全性</h4>
        <p>
          Livery Organizer for FH6 は、GameSave と FH6 本体アセットを <b>読み取り対象</b>として扱います。
          レポート生成や HTML 上の整理操作は、GameSave 内のファイルを削除・移動・リネーム・上書きしません。
          出力先に GameSave 配下を指定した場合は、レポート生成前に拒否します。
        </p>
        <p>
          「削除候補」「基準から消えた」「判定状態を全消去」「古い候補を選択」は、Organizer 側の整理・比較・選択に関する表現です。
          FH6 側のペイントや GameSave を削除する操作ではありません。
        </p>
      </section>
    </div>
  </div>
